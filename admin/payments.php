    ...
    <section class="main-content">
        ...
        <div class="recent-payments-container">
            <table class="main-table">
                ...
                <?php
                        require_once "../db.php";
                        $sql = "SELECT * FROM Payment_Log INNER JOIN Booking ON Payment_Log.Booking_ID = Booking.Booking_ID INNER JOIN User ON Booking.Worker_ID = User.User_ID ORDER BY Timestamp";
                        $result = $conn->query($sql);
                        if($result->num_rows > 0){
                            while($row = $result->fetch_assoc()){
                                $sql_get_customer_name = "SELECT * FROM User WHERE User_ID = ".$row['Customer_ID'];
