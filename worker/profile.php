...
<?php
    ...
    Worker.Worker_ID, 
    P.Plumber_ID, P.Active AS Plumber_Active, 
    C.Carpenter_ID, C.Active AS Carpenter_Active, 
    E.Electrician_ID, E.Active AS Electrician_Active, 
    P2.Painter_ID, P2.Active AS Painter_Active,
    M.Mason_ID, M.Active AS Mason_Active, 
    J.Janitor_ID, J.Active AS Janitor_Active, 
    M2.Mechanic_ID, M2.Active AS Mechanic_Active, 
    G.Gardener_ID, G.Active AS Gardener_Active
FROM 
    Worker
LEFT JOIN Plumber P ON Worker.Worker_ID = P.Plumber_ID
...
LEFT JOIN Gardener G ON Worker.Worker_ID = G.Gardener_ID
WHERE Worker.Worker_ID = {$userId}";
    ...
    <section class="main-content">
        <div class="main-heading">
            ...
            <?php
                ...
                $sql = "SELECT * FROM Login_Attempt WHERE User_ID = {$userId} ORDER BY Current_Timestamp DESC LIMIT 1";
                $result = $conn -> query($sql);

                $row = $result->fetch_assoc();
                $latestTime = date_create($row['Attempt_Time']);
